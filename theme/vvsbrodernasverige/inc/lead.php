 ...
 * or photo. They call vv_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional VVS Bröderna Sverige AB.
 *
 ...
 * @return string
 */
function vv_sample_note() {
	return vv_lead( 'ar_riktig' ) ? '' : ' <span class="vv-sample">(platshållare)</span>';
}
 ...
 * @return string File name for vv_lead_img().
 */
function vv_lead_bild( $i ) {
	$bilder = (array) vv_lead( 'bilder', array() );
 ...
 * @return string
 */
function vv_ring_label() {
	$namn = vv_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, vv_lead( 'telefon' ) ) : 'Ring ' . vv_lead( 'telefon' );
}
