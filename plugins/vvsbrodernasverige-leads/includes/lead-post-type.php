...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'vvsbrodernasverige-leads' ),
					'singular_name' => __( 'Lead', 'vvsbrodernasverige-leads' ),
					'edit_item'     => __( 'Förfrågan', 'vvsbrodernasverige-leads' ),
					'search_items'  => __( 'Search leads', 'vvsbrodernasverige-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'vvsbrodernasverige-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'vvsbrodernasverige-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'vvsbrodernasverige-leads' ),
			'npl_phone'    => __( 'Phone', 'vvsbrodernasverige-leads' ),
			'npl_service'  => __( 'Needs', 'vvsbrodernasverige-leads' ),
			'npl_timeline' => __( 'When', 'vvsbrodernasverige-leads' ),
			'npl_zip'      => __( 'ZIP', 'vvsbrodernasverige-leads' ),
			'npl_source'   => __( 'Source', 'vvsbrodernasverige-leads' ),
			'npl_status'   => __( 'Status', 'vvsbrodernasverige-leads' ),
			'date'         => __( 'Received', 'vvsbrodernasverige-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'vvsbrodernasverige-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'vvsbrodernasverige-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'vvsbrodernasverige-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'vvsbrodernasverige-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'vvsbrodernasverige-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'vvsbrodernasverige-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'vvsbrodernasverige-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'vvsbrodernasverige-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'vvsbrodernasverige-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'vvsbrodernasverige-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'vvsbrodernasverige-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'vvsbrodernasverige-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
